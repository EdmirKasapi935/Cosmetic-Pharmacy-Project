...
class HygieneModel extends ProductModel implements Queryable
{
    private const TABLE_NAME = 'Hygiene'; // Constant for table name
    ...
    {
        // Specific to HygieneModel
        $this->id = $data['ID'] ?? null; // Matches 'ID' in the Hygiene table
    ...
    public function getProducttype() {return "Hygiene";}
    ...
    public function toUpdateQuery()
    {
        ...
        $hygieneUpdateQuery = "UPDATE " . self::TABLE_NAME . " SET 
            Mass = '{$this->mass}', 
            Ingredients = '" . implode(',', $this->ingredients) . "' 
            WHERE ProdID = {$this->prodID};";

        return $productUpdateQuery . " " . $hygieneUpdateQuery;
    }

    public function toDeleteQuery()
    {
        $hygieneDeleteQuery = "DELETE FROM " . self::TABLE_NAME . " WHERE ProdID = {$this->prodID};";
        $productDeleteQuery = "DELETE FROM Products WHERE ProdID = {$this->prodID};";

        return $hygieneDeleteQuery . " " . $productDeleteQuery;
    }
    ...
    public function toCreateQuery()
    {
        ...
        $hygieneCreateQuery = "INSERT INTO " . self::TABLE_NAME . " 
        ...
        return $productCreateQuery . " " . $hygieneCreateQuery;
    }
